...
# eqn = 1, det(r) = 1, then linsolve. Nope.
#
# Next idea. Map {a,b,c,d,e} to integers, and then solve that system of equations.
# Provided we have enough test cases it should work. And in testing it did for R2, ie exp(P5 t)
# Though took 36 min, compared to about a min for the previous method.
# Next, I need to test it on R1, and then R = R2 * R1
#
#
...
max = 5000
count = 0
for f,g,h in product(range(4), repeat=3):
  r0 = str_eqn.replace('f',str(f))
  r1 = r0.replace('g',str(g))
  r2 = r1.replace('h',str(h))
#  print("r2:",r2)

  p0 = det_eqn.replace('f',str(f))
  p1 = p0.replace('g',str(g))
  p2 = p1.replace('h',str(h))
#  print("p2:",p2)

  this_eqn = simplify(sympify(r2) - sympify(p2))
